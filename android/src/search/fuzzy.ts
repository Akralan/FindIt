 ...
 * document : nom, catégorie, résumé. Port direct de
 * `../../../src/lib/search/fuzzy.ts` (webapp PC) — même algorithme, mêmes
 * poids, même comportement. Seule différence : ce module reçoit la liste de
 * documents en paramètre (déjà en mémoire, chargée depuis SQLite) plutôt que
 * d'aller la chercher lui-même, et travaille sur `LocalDocument` plutôt que
 * `DocumentSummary`. Pas d'appel IA, pas d'embedding, pas de dépendance Node.
 */

import type { LocalDocument, SearchResult } from "@/types/document";
...
const FIELD_WEIGHTS: { field: keyof Pick<LocalDocument, "currentName" | "category" | "summary">; weight: number }[] = [
  { field: "currentName", weight: 2 },
  { field: "category", weight: 1.5 },
  { field: "summary", weight: 1 },
];

function scoreDocument(document: LocalDocument, queryTokens: string[]): number {
  let total = 0;
 ...
 * parmi une liste déjà chargée en mémoire (table locale SQLite). Ne renvoie
 * que des documents ayant au moins une correspondance réelle (score > 0) sur
 * le nom, la catégorie ou le résumé — jamais de résultat hors-sujet juste
 * pour remplir la liste. Le score n'a pas de sens en pourcentage : il ne
 * sert qu'au tri, jamais à l'affichage.
 */
export function searchDocuments(documents: LocalDocument[], query: string): SearchResult[] {
